...
- **多生态数据库** — 252,637 条已确认恶意包记录，覆盖 11 个生态系统。
  当前支持 **PyPI**（pip/uv）和 **npm**（npm/pnpm/yarn）代理路由。其他生态
  （RubyGems、NuGet、Crates.io、Go、Maven、Packagist、VSCode）数据库中已有数据，
  但代理路由尚未实现。
...
> **注意：** 当前仅支持 **PyPI**（pip/uv）和 **npm**（npm/pnpm/yarn）代理路由。
> 其他生态在恶意数据库中已有数据，但代理路由尚未实现。
